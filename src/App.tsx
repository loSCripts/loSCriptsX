...
import { Search, Command, Sparkles, ArrowRight, Shield, Globe2, Newspaper, Bot, Settings, X, Plus, ExternalLink, Sun, Moon, Menu, ArrowUpRight, ThumbsUp, User, Clock, Mic, MicOff, FolderOpen, Wrench } from 'lucide-react';
import { fetchTopStories, Story } from './services/hackerNewsApi';
import { performSearch, SearchResult, getSuggestions } from './services/searchApi';
...
function App() {
  ...
  const handleSearchInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Si la barre est vide, on cache tout
    if (!value.trim()) {
      setSearchResults([]);
      setSuggestions([]);
    ...
    // Obtenir les suggestions pendant la saisie
    getSuggestions(value, (newSuggestions) => {
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    setShowSuggestions(false);
    // Lancer la recherche avec la suggestion
  ...
  const handleKeyPress = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      ...
      try {
        const results = await performSearch(searchQuery);
        setSearchResults(results);
        // Scroll vers les résultats après qu'ils sont chargés
        setTimeout(() => {
    ...
    <div className="min-h-screen relative">
      ...
      <main className="flex min-h-screen flex-col items-center justify-center px-4 py-20">
        ...
        {searchResults.length > 0 && (
          <div ref={searchResultsRef} className="mt-4 sm:mt-8 space-y-3 sm:space-y-4 w-full max-w-4xl mx-auto px-2 sm:px-6">
            {searchResults.map((result, index) => (
              <div
                key={index}
                className="bg-gray-900/50 border border-gray-700/50 rounded-lg sm:rounded-xl p-2 sm:p-4 hover:bg-gray-800/50 transition-all duration-300 overflow-hidden"
              >
                <div className="flex items-start gap-2 sm:gap-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-0.5 sm:space-y-1">
                    <h3 className="text-sm sm:text-lg font-semibold text-purple-400 break-words">
                      <a
                        href={result.link}
                        target="_blank"
                        rel="noopener noreferrer"
                      ...
                      >
                        {result.title}
                        <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4 inline-block flex-shrink-0" />
                      </a>
                    </h3>
                    <a
                      href={result.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs sm:text-sm text-gray-400 hover:underline break-words block truncate"
                    >
                      {result.link}
                    </a>
                    <p className="text-xs sm:text-sm text-gray-300 break-words line-clamp-2 sm:line-clamp-3">
                      {result.snippet}
                    </p>
                  </div>
                </div>
      ...
      {showNews && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl p-4 md:p-6 w-full max-w-4xl mx-auto h-[90vh] md:h-[80vh] flex flex-col">
          ...
          <div className="bg-gray-900 rounded-xl p-6 max-w-md w-full mx-4">
            ...
            <form onSubmit={(e) => {
              ...
              <div className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-2 bg-purple-500 hover:bg-purple-600 transition-all duration-300 button-glow"
                >
                  Ajouter
                </button>
              </div>
